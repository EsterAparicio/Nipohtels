...
<html lang="en">
  <head>
    ...
    <title>Delete room - Admintools</title>
            ...
            <div class="row">
              <div class="col-md-10 col-md-offset-1">
                <div class="probootstrap-slider-text text-center">
                  ...
                  <h1 class="probootstrap-heading probootstrap-animate">Delete room</h1>
                  <div class="probootstrap-animate probootstrap-sub-wrap">You only live once, but if you do it right, once is enough.</div>
                </div>
              </div>
            </div>
          </div>
  ...
  <div class="col-md-12 text-center" style="width: 200px; height: 100%; position: relative; left: 850px; margin-top: 50px; margin-bottom: 50px;">
      <form action="droproom.php" method="POST">
        <?php include('../errors.php') ?>
        Hotel (Name):
        <input type="text" name="nombrehotel"/>
        Room number: <input type="number" name="numhab"/>
        <button type="submit" value="Delete room" class="btn btn-default" name="drop_room">Delete room</button>
      </form>
      <?php
      if (isset($_POST['drop_room'])) {
      $nombrehotel = $_POST['nombrehotel'];
      $numhab = $_POST['numhab'];
      if (empty($nombrehotel)) {
            array_push($errors, "Es necesario conocer el nombre del hotel del cual se va a eliminar la habitación");
        }
        if (empty($numhab)) {
            array_push($errors, "Es necesario saber el número de habitación que se va a eliminar");
        }
      if (count($errors) == 0) {
        $queryhotel = "SELECT id_hotel FROM hoteles WHERE nombre = '$nombrehotel'";
          ...
          $idhotel = $row[0];
          $query = "DELETE FROM habitaciones WHERE hoteles_id_hotel = '$idhotel' AND num_habitacion = '$numhab'";
