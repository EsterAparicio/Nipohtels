...
<html lang="en">
  <head>
    ...
    <title>Register hotel pensions - Admintools</title>
            ...
            <div class="row">
              <div class="col-md-10 col-md-offset-1">
                <div class="probootstrap-slider-text text-center">
                  ...
                  <h1 class="probootstrap-heading probootstrap-animate">Create hotel pensions</h1>
                  <div class="probootstrap-animate probootstrap-sub-wrap">Traveling is not something you're good at. It's something you do, like breathing.</div>
                </div>
              </div>
            </div>
          </div>
  ...
  <div class="col-md-12 text-center" style="width: 200px; height: 100%; position: relative; left: 850px; margin-top: 50px; margin-bottom: 50px;">
      <form method="POST" action="newpension.php" enctype="multipart/form-data">
    ...
    <div class="input-group">
      ...
      <div class="input-group">
        ...
        <input type="number" step="10" name="breakfast">
      </div>
      <div class="input-group">
        <label>Half board:</label>
        <input type="number" step="10" name="halfboard">
      </div>
      <div class="input-group">
        <label>Full board:</label>
        <input type="number" step="10" name="fullboard">
      </div>
      <div class="input-group">
        <label>All included:</label>
        <input type="number" step="10" name="allincluded">
        <hr/>
      </div>
      <div class="input-group">
        <button type="submit" style="margin-left: 25px;" class="btn" name="updatepensions">Register pensions</button>
      </div>
  </form>
  <?php 
      if (isset($_POST['updatepensions'])) {
    $hotelname = mysqli_real_escape_string($db, $_POST['name']);
    $breakfast = mysqli_real_escape_string($db, $_POST['breakfast']);
    $halfboard = mysqli_real_escape_string($db, $_POST['halfboard']);
    $fullboard = mysqli_real_escape_string($db, $_POST['fullboard']);
    $allincluded = mysqli_real_escape_string($db, $_POST['allincluded']);

    if (empty($hotelname)) {
      array_push($errors, "A name for the hotel is required");
    }
    if (empty($breakfast)) {
      array_push($errors, "A price for breakfast is required");
    }
    if (empty($halfboard)) {
      array_push($errors, "A price for half board is required");
    }
    if (empty($fullboard)) {
      array_push($errors, "A price for full board is required");
    }
    if (empty($allincluded)) {
      array_push($errors, "A price for all included is required");
    }


    if (count($errors) == 0) {
      $query = mysqli_query($db,"SELECT id_hotel FROM hoteles WHERE nombre = '$hotelname';");
      $row = mysqli_fetch_row($query);
      $idhotel = $row[0];

      //NADA INCLUIDO
      $querynothing = mysqli_query($db,"INSERT INTO `tipopensiones_hoteles` (`tipo_pensiones_id_pension`, `hoteles_id_hotel`, `precio_final_pension`) VALUES ('1', '$idhotel', '0')") or die(mysqli_error($db));

      //DESAYUNO
      $querybreakfast = mysqli_query($db,"INSERT INTO `tipopensiones_hoteles` (`tipo_pensiones_id_pension`, `hoteles_id_hotel`, `precio_final_pension`) VALUES ('2', '$idhotel', '$breakfast')") or die(mysqli_error($db));

      //MEDIA PENSION
      $queryhalfboard = mysqli_query($db,"INSERT INTO `tipopensiones_hoteles` (`tipo_pensiones_id_pension`, `hoteles_id_hotel`, `precio_final_pension`) VALUES ('3', '$idhotel', '$halfboard')") or die(mysqli_error($db));

      //PENSION COMPLETA
      $queryfullboard = mysqli_query($db,"INSERT INTO `tipopensiones_hoteles` (`tipo_pensiones_id_pension`, `hoteles_id_hotel`, `precio_final_pension`) VALUES ('4', '$idhotel', '$fullboard')") or die(mysqli_error($db));

      //TODO INCLUIDO
      $queryallincluded = mysqli_query($db,"INSERT INTO `tipopensiones_hoteles` (`tipo_pensiones_id_pension`, `hoteles_id_hotel`, `precio_final_pension`) VALUES ('5', '$idhotel', '$allincluded')") or die(mysqli_error($db));
