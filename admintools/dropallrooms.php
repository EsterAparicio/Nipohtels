...
<html lang="en">
  <head>
    ...
    <title>Delete all rooms from an hotel - Admintools</title>
            ...
            <div class="row">
              <div class="col-md-10 col-md-offset-1">
                <div class="probootstrap-slider-text text-center">
                  ...
                  <h1 class="probootstrap-heading probootstrap-animate">Delete all rooms from an hotel</h1>
                  <div class="probootstrap-animate probootstrap-sub-wrap">If your ship doesn't come in, swim out to it.</div>
                </div>
              </div>
            </div>
          </div>
  ...
  <div class="col-md-12 text-center" style="width: 200px; height: 100%; position: relative; left: 850px; margin-top: 50px; margin-bottom: 50px;">
      <form action="dropallrooms.php" method="POST">
        <?php include('../errors.php') ?>
        Hotel (Name):
        <input type="text" name="nombrehotel"/>
        <button type="submit" value="Delete all rooms" class="btn btn-default" name="drop_all_rooms">Delete all rooms</button>
      </form>
      <?php
      if (isset($_POST['drop_all_rooms'])) {

      $nombrehotel = $_POST['nombrehotel'];
      if (empty($nombrehotel)) {
            array_push($errors, "Es necesario conocer el nombre del hotel del cual se van a eliminar las habitaciones");
        }
      ...
      if (count($errors) == 0) {
        $queryhotel = "SELECT id_hotel FROM hoteles WHERE nombre = '$nombrehotel'";
          ...
          $idhotel = $row[0];
          $query = "DELETE FROM habitaciones WHERE hoteles_id_hotel = '$idhotel'";
