...
<html lang="en">
  <head>
    ...
    <title>Register new room - Admintools</title>
            ...
            <div class="row">
              <div class="col-md-10 col-md-offset-1">
                <div class="probootstrap-slider-text text-center">
                  ...
                  <h1 class="probootstrap-heading probootstrap-animate">Create new room</h1>
                  <div class="probootstrap-animate probootstrap-sub-wrap">All we have to decide is what to do with the time that is given us.</div>
                </div>
              </div>
            </div>
          </div>
  ...
  <div class="col-md-12 text-center" style="width: 200px; height: 100%; position: relative; left: 850px; margin-top: 50px; margin-bottom: 50px;">
    <form method="POST" action="newroom.php" enctype="multipart/form-data">
    ...
    <div class="input-group">
      ...
      <div class="input-group">
        <label>Number of beds (1 - 5):</label>
        <input type="text" name="camas" onblur="checkBeds()" id="beds">
      </div>
      <div class="input-group">
      ...
      <div class="input-group">
        <button type="submit" class="btn" name="new_room" id="submitroom">Create room</button>
      </div>
  </form>
  <?php     if (isset($_POST['new_room'])) {
    $nombrehotel = mysqli_real_escape_string($db, $_POST['nombre']);
    $idalo = mysqli_real_escape_string($db, $_POST['camas']);
    $descripcion = mysqli_real_escape_string($db, $_POST['descripcion']);
    $numhab = mysqli_real_escape_string($db, $_POST['numhab']);

    if (empty($nombrehotel)) {
      array_push($errors, "It's necessary to know in which hotel will it be created.");
    }
    if (empty($idalo)) {
      array_push($errors, "It's necessary to know the number of beds in the room.");
    }
    if (empty($descripcion)) {
      array_push($errors, "It's necessary to add a brief description of the room.");
    }
    if (empty($numhab)) {
      array_push($errors, "It's necessary to know the room number.");
    }
    ...
    $path = "/../img/rooms/" . basename($imagename);
  //if (move_uploaded_file($imagefile['tmp_name'], $path)) {
    if (move_uploaded_file($_FILES['image']['tmp_name'], SITE_ROOT.$path)) {
      ...
      array_push($errors, "There's something wrong with that image...");
  }
  if (empty($imagefile)) {
      array_push($errors, "An image for the room is required");
    }

    if (count($errors) == 0) {

      $queryhotel = "SELECT id_hotel FROM hoteles WHERE nombre = '$nombrehotel'";
      $resultshotel = mysqli_query($db, $queryhotel) or die(mysql_error());
      $row = mysqli_fetch_row($resultshotel);
      $idhotel = $row[0];
      $query = "INSERT INTO habitaciones (`id_habitacion`, `descripcion`, `img_path`, `num_habitacion`, `hoteles_id_hotel`, `tipo_alojamiento_id_alojamiento`, `tipo_pensiones_id_pension`) VALUES ('', '$descripcion','$imagename', '$numhab', '$idhotel', '$idalo', '1')";
...
<section class="probootstrap-half">
  ...
  <script type="text/javascript">
      function checkBeds(){
        if( document.getElementById('beds').value > 5 || document.getElementById('beds').value < 1){
          array_push($errors, "There has to be between 1 and 5 beds in the room");
        }
    }
  </script>
