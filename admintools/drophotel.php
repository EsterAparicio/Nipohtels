...
<html lang="en">
  <head>
    ...
    <title>Delete hotel - Admintools</title>
            ...
            <div class="row">
              <div class="col-md-10 col-md-offset-1">
                <div class="probootstrap-slider-text text-center">
                  ...
                  <h1 class="probootstrap-heading probootstrap-animate">Delete a hotel</h1>
                  <div class="probootstrap-animate probootstrap-sub-wrap">Without new experiences, something inside us sleeps. The sleeper must awaken.</div>
                </div>
              </div>
            </div>
          </div>
  ...
  <div class="col-md-12 text-center" style="width: 200px; height: 100%; position: relative; left: 850px; margin-top: 50px; margin-bottom: 50px;">
    <div class="search-div">
      <form action="drophotel.php" method="POST">
        <?php include('../errors.php') ?>
        Hotel (Name):
        <input type="text" name="nombrehotel"/>
        <button type="submit" value="Delete hotel" class="btn btn-default" name="drophotel">Delete hotel</button>
      </form>
      <?php
          if (isset($_POST['drophotel'])) {

      $nombrehotel = $_POST['nombrehotel'];
      if (empty($nombrehotel)) {
            array_push($errors, "It's necessary to know the name of the hotel you're about to erase.");
        }

      if (count($errors) == 0) {
        //Elimina las habitaciones antes de eliminar el hotel
        $queryhotel = "SELECT id_hotel FROM hoteles WHERE nombre = '$nombrehotel'";
          ...
          $idhotel = $row[0];
          $queryhabitaciones = "DELETE FROM habitaciones WHERE hoteles_id_hotel = '$idhotel'";
          $resultshabitaciones = mysqli_query($db, $queryhabitaciones) or die(mysqli_error($db));

          //Elimina el hotel
          $query = "DELETE FROM hoteles WHERE nombre = '$nombrehotel'";
          $results = mysqli_query($db, $query) or die(mysqli_error($db));
          echo "Hotel deleted correctly";
