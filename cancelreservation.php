...
$errors = array();
if (isset($_POST['drop_reservation'])) {
      $correo = $_SESSION['login_user'];
      $fechaentrada = $_POST['fechaentrada'];
      $fechasalida = $_POST['fechasalida'];
        if (empty($fechaentrada)) {
            array_push($errors, "Una fecha de entrada es necesaria");
        }
        if (empty($fechasalida)) {
            array_push($errors, "Una fecha de salida es necesaria");
        }
      if (count($errors) == 0) {
          $query = "DELETE FROM reservas WHERE usuarios_correo_usuario = '$correo' AND fecha_reserva_inicio = '$fechaentrada' AND fecha_reserva_fin = '$fechasalida'";
          $results = mysqli_query($db, $query) or die(mysqli_error($db));
          echo "<h1 class='cancel-reservation'>Reservation successfully cancelled</h1>";
...
<html lang="en">
  <head>
    ...
    <title>Cancel a reservation - Nipohtels</title>
            ...
            <div class="row">
              <div class="col-md-10 col-md-offset-1">
                <div class="probootstrap-slider-text text-center">
                  ...
                  <h1 class="probootstrap-heading probootstrap-animate">Cancel reservation</h1>
                  <div class="probootstrap-animate probootstrap-sub-wrap">Live with no excuses and travel with no regrets.</div>
                </div>
              </div>
            </div>
          </div>
  ...
  <div class="form-rate">
      <form action="cancelreservation.php" method="POST">
        ...
        Start date: <input type="date" name="fechaentrada"/>
        End date: <input type="date" name="fechasalida"/>
        <button type="submit" value="Cancel reservation" class="btn btn-default" name="drop_reservation">Cancel reservation</button>
      </form>
  </div>
  </section>
