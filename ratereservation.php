...
$errors = array();
    ...
    if (isset($_POST['rate_reservation'])) {
      $tripstart = $_POST['datestart'];
      $tripend = $_POST['dateend'];
      $rate = $_POST['rate'];
      if (empty($tripstart)) {
            array_push($errors, "The start date is necessary.");
        }
        if (empty($tripend)) {
            array_push($errors, "The end date is necessary.");
        }
        if (empty($rate)) {
            array_push($errors, "It's necessary to add a rate.");
        }

        //Comprobar que esas fechas son de una reserva existente
        $sqlreserva = "SELECT * FROM reservas WHERE usuarios_correo_usuario = '$correo' AND fecha_reserva_inicio = '$tripstart' AND fecha_reserva_fin = '$tripend';";
        $queryreserva = mysqli_query($db, $sqlreserva) or die(mysqli_error($db));
        $arrayreserva = mysqli_fetch_array($queryreserva);
        if (count($arrayreserva) == 0) {
          array_push($errors, "The date entered does not match any reservation.");
        }

        if (count($errors) == 0) {
          $sql = "INSERT INTO valoraciones (`fecha`, `valor`, `usuarios_correo`, `reservas_fecha_reserva`) VALUES ('$fecha', '$rate', '$correo','$tripstart');";
          $query = mysqli_query($db, $sql);

          $sql2 = "SELECT id_valoracion FROM valoraciones WHERE fecha = '$fecha' AND usuarios_correo = '$correo' AND reservas_fecha_reserva = '$tripstart'";
          $query2 = mysqli_query($db, $sql2);
          $row = mysqli_fetch_row($query2);

          $sql3 = "UPDATE reservas SET valoraciones_id_valoracion = '$row[0]'
          WHERE usuarios_correo_usuario = '$correo' AND fecha_reserva_inicio = '$tripstart' AND fecha_reserva_fin = '$tripend';";
          $query3 = mysqli_query($db, $sql3) or die(mysqli_error($db));

          echo "Valuation inserted successfully.";
        }
    }
  ?>
<!DOCTYPE html>
<html lang="en">
  <head>
    ...
    <title>Rate a reservation - Nipohtels</title>
    ...
    <!-- <div class="container"> -->
        ...
        <nav role="navigation" class="probootstrap-nav hidden-xs">
          ...
          <ul class="probootstrap-main-nav">
            ...
            <li class="hidden-xs probootstrap-logo-center"><a href="index.php"><img src="img/logo_md.png" class="hires" width="181" height="50" alt="Nipohtels logo"></a></li>
            <li><a href="contact.php">Contact</a></li>
            <li><a href='adminindex.php'>Admin Tools</a></li>
            <li><a href='logout.php'>Log out</a></li>
          </ul>
          <?php } ?>
            ...
            <div class="row">
              <div class="col-md-10 col-md-offset-1">
                <div class="probootstrap-slider-text text-center">
                  ...
                  <h1 class="probootstrap-heading probootstrap-animate">Rate a reservation</h1>
                  <div class="probootstrap-animate probootstrap-sub-wrap">To travel is to discover that everyone is wrong about other countries.</div>
                </div>
              </div>
            </div>
          </div>
  ...
  <div class="form-rate">
  <form action="ratereservation.php" method="POST">
    <?php include ('errors.php') ?>
    From: <input type="date" name="datestart" id="tripStart" value=""/>
    To: <input type="date" name="dateend" id="tripEnd" value=""/>
    Rate: <input type="number" name="rate">
    <button type="submit" class="btn btn-default" value="Search" name="rate_reservation">Search</button>
  </form>
  </div>
  </section>
