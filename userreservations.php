...
<html lang="en">
  <head>
    ...
    <title>See my reservations - Nipohtels</title>
            ...
            <div class="row">
              <div class="col-md-10 col-md-offset-1">
                <div class="probootstrap-slider-text text-center">
                  ...
                  <h1 class="probootstrap-heading probootstrap-animate">See my reservations</h1>
                  <div class="probootstrap-animate probootstrap-sub-wrap">The world is changed by your example, not your opinion.</div>
                </div>
              </div>
            </div>
          </div>
  ...
  <section class="table-responsive">
    <table class="user-table-reservations">
      <?php     
        ...
        INNER JOIN habitaciones_reservas ON habitaciones_reservas.reservas_id_reserva = id_reserva
        INNER JOIN habitaciones ON habitaciones_reservas.habitaciones_id_habitacion = habitaciones.id_habitacion
        INNER JOIN hoteles ON hoteles.id_hotel = habitaciones.hoteles_id_hotel
        WHERE reservas.usuarios_correo_usuario = '$correo' GROUP BY id_reserva ORDER BY fecha_reserva_inicio";
        //$query = "SELECT * FROM reservas WHERE usuarios_correo_usuario >= '$correo' ORDER BY fecha_reserva_inicio";
        $raw_results = mysqli_query($db, $query);

    if(mysqli_num_rows($raw_results) > 0){
      echo "<tr><th>Hotel</th><th>Start date</th><th>End date</th><th>Final price</th></tr>";
             
            while($results = mysqli_fetch_array($raw_results)){
                echo "<tr>";
                echo "<td> ".$results['nombre']."</td>";             
                echo "<td> ".$results['fecha_reserva_inicio']."</td>";
                echo "<td> ".$results['fecha_reserva_fin']."</td>";
                echo "<td> ".$results['precio_final_reserva']." €</td>";
                echo "</tr>";
            }
        } ?>
      </table>
